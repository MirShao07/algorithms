...
def turn_to_graph(input,start,end,out,lap):
    outlets = {}
    laptops = {}
    adapters = set()
    for i in range(start+1,start+out+1):
        outlets[input[i]] = outlets.get(input[i],0) + 1
    for j in range(start + out + 2, start + out + lap + 2):
        laptops[input[j]] = laptops.get(input[j],0) + 1
    for k in range(start + out + lap + 3,end):
        pair = input[k].split()
...
def main():
    ...
    res = []
    for i in range(case):
        out_num = int(lst[offset+0])
        laptop_num = int(lst[offset+out_num+1])
        adapter_num = int(lst[offset+out_num+laptop_num+2])
        end = offset + out_num + laptop_num + adapter_num + 3
        graph = turn_to_graph(lst,offset,end,out_num,laptop_num)
        offset = end
        max_flow = calculate_maxflow(graph)
        res.append(laptop_num - max_flow)
